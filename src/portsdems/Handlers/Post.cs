		...
		public static async Task ProcessRequestAsync(HttpContext Context, string Action) {
			...
			switch (Action) {
				case Forms.Subscribe.Action.MailingList:
					string email = form[Forms.Subscribe.Keys.EmailAddress];
					if (email != null) {
                        bool alreadySubscribed = false;
						using (SqlConnection target = new SqlConnection(Application.Configuration.DBConnectionString)) {
                            try {
